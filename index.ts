import * as fourtwentyUtil from 'fourtwentyjs-util';
import * as fourtwentyAbi from 'fourtwentyjs-abi';
...
export interface FourtwentyEncryptedData {
  version: string;
  nonce: string;
...
const TypedDataUtils = {
  ...
  encodeData (primaryType: string, data: object, types: object, useV4 = true): Buffer {
      ...
      const encodeField = (name, type, value) => {
        if (types[type] !== undefined) {
          ...
          return ['bytes32', value == null ?
            ...
            fourtwentyUtil.sha3(this.encodeData(type, value, types, useV4))];
        }
        ...
        if (type === 'bytes') {
          return ['bytes32', fourtwentyUtil.sha3(value)];
        }

        if (type === 'string') {
          // convert string to buffer - prevents fourtwentyUtil from interpreting strings like '0xabcd' as hex
        ...
        if (type.lastIndexOf(']') === type.length - 1) {
          ...
          return ['bytes32', fourtwentyUtil.sha3(fourtwentyAbi.rawEncode(
      ...
      for (const field of types[primaryType]) {
        ...
        if (value !== undefined) {
          if (field.type === 'bytes') {
            ...
            value = fourtwentyUtil.sha3(value);
            encodedValues.push(value);
            ...
            encodedTypes.push('bytes32');
            value = fourtwentyUtil.sha3(this.encodeData(field.type, value, types, useV4));
            encodedValues.push(value);
    ...
    return fourtwentyAbi.rawEncode(encodedTypes, encodedValues);
  },
  ...
  hashStruct (primaryType: string, data: object, types: object, useV4 = true): Buffer {
    return fourtwentyUtil.sha3(this.encodeData(primaryType, data, types, useV4));
  },
  ...
  hashType (primaryType: string, types: object): Buffer {
    return fourtwentyUtil.sha3(this.encodeType(primaryType, types));
  },
  ...
  sign<T extends MessageTypes> (typedData: Partial<TypedData | TypedMessage<T>>, useV4 = true): Buffer {
    ...
    return fourtwentyUtil.sha3(Buffer.concat(parts));
  },
};

function concatSig (v: Buffer, r: Buffer, s: Buffer): string {
  const rSig = fourtwentyUtil.fromSigned(r);
  const sSig = fourtwentyUtil.fromSigned(s);
  const vSig = fourtwentyUtil.bufferToInt(v);
  const rStr = padWithZeroes(fourtwentyUtil.toUnsigned(rSig).toString('hex'), 64);
  const sStr = padWithZeroes(fourtwentyUtil.toUnsigned(sSig).toString('hex'), 64);
  const vStr = fourtwentyUtil.stripHexPrefix(fourtwentyUtil.intToHex(vSig));
  return fourtwentyUtil.addHexPrefix(rStr.concat(sStr, vStr)).toString('hex');
}
...
function normalize (input: number | string): string {
  ...
  if (typeof input === 'number') {
    const buffer = fourtwentyUtil.toBuffer(input);
    input = fourtwentyUtil.bufferToHex(buffer);
  }

  if (typeof input !== 'string') {
    let msg = 'fourtwenty-sig-util.normalize() requires hex string or integer input.';
    msg += ` received ${typeof input}: ${input}`;
    throw new Error(msg);
  }

  return fourtwentyUtil.addHexPrefix(input.toLowerCase());
}

function personalSign<T extends MessageTypes> (privateKey: Buffer, msgParams: MsgParams<TypedData | TypedMessage<T>>): string {
  const message = fourtwentyUtil.toBuffer(msgParams.data);
  const msgHash = fourtwentyUtil.hashPersonalMessage(message);
  const sig = fourtwentyUtil.ecsign(msgHash, privateKey);
  const serialized = fourtwentyUtil.bufferToHex(concatSig(sig.v, sig.r, sig.s));
  return serialized;
}
...
function recoverPersonalSignature<T extends MessageTypes> (msgParams: SignedMsgParams<TypedData | TypedMessage<T>>): string {
  ...
  const sender = fourtwentyUtil.publicToAddress(publicKey);
  const senderHex = fourtwentyUtil.bufferToHex(sender);
  return senderHex;
}
...
function externalTypedSignatureHash (typedData: EIP712TypedData[]): string {
  ...
  return fourtwentyUtil.bufferToHex(hashBuffer);
}
...
function signTypedDataLegacy<T extends MessageTypes> (privateKey: Buffer, msgParams: MsgParams<TypedData | TypedMessage<T>>): string {
  ...
  const sig = fourtwentyUtil.ecsign(msgHash, privateKey);
  return fourtwentyUtil.bufferToHex(concatSig(sig.v, sig.r, sig.s));
}
...
function recoverTypedSignatureLegacy<T extends MessageTypes> (msgParams: SignedMsgParams<TypedData | TypedMessage<T>>): string {
  ...
  const sender = fourtwentyUtil.publicToAddress(publicKey);
  return fourtwentyUtil.bufferToHex(sender);
}

function encrypt<T extends MessageTypes> (receiverPublicKey: string, msgParams: MsgParams<TypedData | TypedMessage<T>>, version: string): FourtwentyEncryptedData {
...
function encryptSafely<T extends MessageTypes> (receiverPublicKey: string, msgParams: MsgParams<TypedData | TypedMessage<T>>, version: string): FourtwentyEncryptedData {
...
function decrypt (encryptedData: FourtwentyEncryptedData, receiverPrivateKey: string): string {
...
function decryptSafely (encryptedData: FourtwentyEncryptedData, receiverPrivateKey: string): string {
...
function signTypedData<T extends MessageTypes> (privateKey: Buffer, msgParams: MsgParams<TypedData | TypedMessage<T>>): string {
  ...
  const sig = fourtwentyUtil.ecsign(message, privateKey);
  return fourtwentyUtil.bufferToHex(concatSig(sig.v, sig.r, sig.s));
}
...
function typedSignatureHash<T extends MessageTypes> (typedData: TypedData | TypedMessage<T>): Buffer {
  ...
  const data = typedData.map(function (e) {
    return e.type === 'bytes' ? fourtwentyUtil.toBuffer(e.value) : e.value;
  });
  ...
  return fourtwentyAbi.soliditySHA3(
    ['bytes32', 'bytes32'],
    [
      fourtwentyAbi.soliditySHA3(new Array(typedData.length).fill('string'), schema),
      fourtwentyAbi.soliditySHA3(types, data),
    ]
  );
}

function recoverPublicKey (hash: Buffer, sig: string): Buffer {
  const signature = fourtwentyUtil.toBuffer(sig);
  const sigParams = fourtwentyUtil.fromRpcSig(signature);
  return fourtwentyUtil.ecrecover(hash, sigParams.v, sigParams.r, sigParams.s);
}
